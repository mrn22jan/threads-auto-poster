...
def post_to_threads(text, reply_to_id=None):
    ...
    try:
        ...
        time.sleep(35) # Threads側の処理待ち（少し長めに設定）
...
st.title("🧵 Threadsツリー完全管理システム [決定版]")
            ...
            for idx, txt in enumerate(texts):
                if idx > 0:
                    for t in range(300, 0, -1):
                        prog.warning(f"⏳ 【テスト】ツリー待機中 ({idx}/{len(texts)}) \n 次まであと **{t}** 秒...")
                        time.sleep(1)
                prog.info(f"📤 {idx+1}本目を投稿中...")
                ok, res = post_to_threads(txt, tid)
                if ok: tid = res; st.write(f"✅ {idx+1}本目 成功")
                else: st.error(f"❌ 失敗: {res}"); break
            st.success("完了！")
...
for i, r in enumerate(data_rows, start=2):
    ...
    if r[0] and "完了" not in status:
        ...
        if idx < len(allowed) and idx < new_m:
            ...
            schedule.append({"row": i, "time": jst_now.replace(hour=h, minute=m, second=0, microsecond=0), "data": r, "status": status})
...
if schedule:
    ...
    if jst_now >= task["time"] and (time_gap >= 3600 or is_resuming):
        ...
        if is_resuming:
            ...
            current_tid = task["data"][7] if len(task["data"]) > 7 else None # H列からID取得
        ...
        for idx in range(start_idx, len(texts)):
            ...
            if ok:
                ...
                # 再開用にIDをH列に保存
                sheet.update_cell(task["row"], 8, current_tid)
